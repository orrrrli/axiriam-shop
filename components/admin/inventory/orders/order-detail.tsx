...
import { OrderMaterial, OrderMaterialFormData, OrderMaterialStatus, RawMaterial } from '@/types/inventory';
...
export default function OrderDetail({ order, materialsById }: OrderDetailProps): React.ReactElement {
  ...
  const { saving: updating, update } = useOrderMutations();
  ...
  async function handleStatusChange(status: OrderMaterialStatus): Promise<void> {
    ...
    const result = await update(order.id, { ...order, status } as OrderMaterialFormData);
    if (!result.success) {
  ...
  async function handleSaveTracking(): Promise<void> {
    ...
    const result = await update(order.id, { ...order, trackingNumber: trackingInput.trim() } as OrderMaterialFormData);
    if (result.success) {
