  ...
  const columns: Column<StoreOrder>[] = [
    {
      ...
      render: (value: unknown) => {
        ...
        return <span className="font-mono text-[1.2rem] text-subtle">#{id.slice(-8).toUpperCase()}</span>;
      },
    },
    {
      header: 'Cliente',
      key: 'customer',
      render: (value: unknown) => {
        const customer = value as StoreOrder['customer'];
        return (
          <div>
            <p className="font-bold text-heading text-[1.3rem]">{customer.name}</p>
            <p className="text-subtle text-[1.1rem]">{customer.email}</p>
          </div>
        );
      },
    },
    {
      ...
      key: 'orderItems',
      render: (value: unknown) => {
        const items = value as StoreOrder['orderItems'];
        return (
    ...
    },
    {
      header: 'Pago',
      key: 'isPaid',
      render: (value: unknown, row: StoreOrder) => {
        const paid = value as boolean;
        return (
          <div>
            <span className={`inline-block px-[0.8rem] py-[0.2rem] text-[1.1rem] font-semibold rounded-[0.4rem] ${paid ? 'bg-[#e8f5e9] text-[#2e7d32]' : 'bg-[#fce4ec] text-[#c62828]'}`}>
    ...
    },
    {
      ...
      key: 'status',
      render: (value: unknown, row: StoreOrder) => {
        const status = value as StoreOrderStatus;
        return (
          <select
            className={`text-[1.2rem] font-bold px-[0.8rem] py-[0.3rem] border-0 cursor-pointer focus:outline-none rounded-[0.4rem] ${VENTA_STATUS_STYLES[status]}`}
            value={status}
    ...
    },
    {
      ...
      key: 'createdAt',
      render: (value: unknown) => (
        <span className="text-subtle text-[1.2rem]">{formatDate(value as Date)}</span>
      ),
    },
  ];
